...
def run_sentiment_analysis(test_reviews):
    g = open('scripts/sentiment/reviews.txt', 'r')
    reviews = list(map(lambda x: x[:-1], g.readlines()))
    g.close()

    g = open('scripts/sentiment/labels.txt', 'r')
    ...
    if not (os.path.isfile('scripts/sentiment/weights_layer_0_1.dat') and os.path.isfile('scripts/sentiment/weights_layer_0_2.dat')):
        weights_layer_1, weights_layer_2 = mlp.train(
           reviews[:-1000], labels[:-1000]
        )
        print('Training has finished')
        weights_layer_1.dump("scripts/sentiment/weights_layer_0_1.dat")
        weights_layer_2.dump("scripts/sentiment/weights_layer_0_2.dat")
        print('Hyper parameters have been saved')


    weights_0_1 = np.load("scripts/sentiment/weights_layer_0_1.dat")
    weights_0_2 = np.load("scripts/sentiment/weights_layer_0_2.dat")
